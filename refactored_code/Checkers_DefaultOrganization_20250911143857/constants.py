...
def get_board_size():
    """
    Retrieve the total board size in pixels.

    Returns:
        int: The side length of the entire board in pixels.
    """
    return BOARD_SIZE
...
def get_panel_height():
    """
    Retrieve the height of the control panel below the board.
...
def get_fps():
    """
    Retrieve the target frames per second for the application.
...
def get_color(color_name):
    ...
    Returns:
        tuple: An RGB color tuple, or None if the color name is not found.

    Raises:
        KeyError: If the specified color name does not exist in the palette.
    """
    palette = get_color_palette()
    if color_name not in palette:
        logger.warning(f"Color '{color_name}' not found in palette")
