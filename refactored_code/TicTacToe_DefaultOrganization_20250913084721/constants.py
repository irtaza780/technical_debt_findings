...
class ColorScheme(Enum):
    ...
    SYMBOL_X_FOREGROUND = '#e53935'  # Red for X
    SYMBOL_O_FOREGROUND = '#1e88e5'  # Blue for O
...
def get_color(color_name: str) -> str:
    """
    Retrieve a color value from the color scheme.
    
    Args:
        color_name: The name of the color to retrieve (e.g., 'bg', 'x_fg').
    ...
    color_mapping = {
        ...
        'x_fg': ColorScheme.SYMBOL_X_FOREGROUND.value,
        'o_fg': ColorScheme.SYMBOL_O_FOREGROUND.value,
        'highlight': ColorScheme.HIGHLIGHT.value,
...
def get_font(font_type: str) -> Tuple[str, int, str]:
    """
    Retrieve a font configuration tuple.
    
    Args:
        font_type: The type of font ('cell', 'status', or 'control').
    ...
    if font_type not in font_configs:
        logger.error(f"Font type '{font_type}' not found in font configurations")
