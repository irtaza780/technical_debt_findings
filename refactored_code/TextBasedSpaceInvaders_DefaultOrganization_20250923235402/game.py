...
class Game:
    ...
    def new_game(self):
        """Reset all game state and spawn player and aliens for a new game."""
    ...
    def _spawn_aliens(self):
        """Create and add alien sprites in a grid formation."""
        grid_width = settings.ALIEN_COLS * settings.ALIEN_WIDTH + (settings.ALIEN_COLS - 1) * settings.ALIEN_X_SPACING
        ...
        for row in range(settings.ALIEN_ROWS):
            ...
            for col in range(settings.ALIEN_COLS):
                alien = Alien(x, y, color_index=row)
    ...
    def handle_events(self):
        """Process pygame events including quit, keyboard input, and shooting."""
        ...
        # Handle continuous shooting during gameplay
        if self.state == GAME_STATE_PLAYING:
            self._handle_shooting()

    def _handle_shooting(self):
        """Process player shooting input and add bullets to the game."""
    ...
    def update(self):
        """Update game state: entities, collisions, and win/lose conditions."""
        if self.state == GAME_STATE_PLAYING:
            keys = pygame.key.get_pressed()
    ...
    def _resolve_collisions(self):
        """Handle all collision detection: bullets hitting aliens and aliens hitting player."""
        self._resolve_bullet_alien_collisions()
        self._resolve_player_alien_collisions()

    def _resolve_bullet_alien_collisions(self):
        """Detect and handle bullets hitting aliens."""
    ...
    def _resolve_player_alien_collisions(self):
        """Detect and handle aliens colliding with player (with invulnerability check)."""
        now = pygame.time.get_ticks()
    ...
    def _draw_lives(self):
        """Render and display remaining lives as ship icons."""
    ...
    def _draw_state_overlay(self):
        """Draw overlay messages based on current game state."""
    ...
    def _draw_centered_message(self, title: str, subtitle: str):
        ...
        subtitle_rect = subtitle_surf.get_rect(
            center=(settings.WIDTH // 2, settings.HEIGHT // 2 + CENTERED_MESSAGE_SUBTITLE_OFFSET_Y)
        )
    ...
    def run(self):
        """Main game loop: handle events, update state, and render each frame."""
        while self.running:
            self.handle_events()
            self.update()
            self.draw()