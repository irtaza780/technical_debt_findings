...
class ReversiGUI:
    ...
    def _setup_buttons(self) -> None:
        ...
        self.new_game_btn.pack(pady=(BUTTON_PADDING_TOP, BUTTON_PADDING))
        ...
        self.undo_btn.pack(pady=BUTTON_PADDING)
        ...
        self.quit_btn.pack(pady=BUTTON_PADDING)
    ...
    def _draw_discs(self) -> None:
        ...
        for row in range(BOARD_SIZE):
            for col in range(BOARD_SIZE):
                ...
                self._draw_single_disc(row, col, cell)

    def _draw_single_disc(self, row: int, col: int, cell_value: int) -> None:
        """Draw a single disc on the board.
        
        Args:
            row: The row position on the board.
            col: The column position on the board.
            cell_value: The cell value (BLACK or WHITE).
        """
        x0, y0, x1, y1 = self.board_to_canvas_coords(row, col)

        # Determine disc color based on cell value
        if cell_value == ReversiGame.BLACK:
    ...
    def _draw_valid_move_hints(self) -> None:
        """Draw hint circles for all valid moves available to the current player."""
        ...
        for row, col in moves:
            self._draw_hint_circle(row, col, hint_color)

    def _draw_hint_circle(self, row: int, col: int, color: str) -> None:
        """Draw a hint circle at the specified board position.
        
        Args:
            row: The row position on the board.
            col: The column position on the board.
            color: The color of the hint circle.
        """
        x0, y0, x1, y1 = self.board_to_canvas_coords(row, col)
        center_x = (x0 + x1) / 2
        center_y = (y0 + y1) / 2
    ...
    def board_to_canvas_coords(self, row: int, col: int) -> Tuple[int, int, int, int]:
    ...
    def _show_game_over(self) -> None:
        ...
        logger.info(f"Game over. {result_message} Final score - Black: {score['black']}, White: {score['white']}")