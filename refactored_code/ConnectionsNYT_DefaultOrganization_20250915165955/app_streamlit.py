...
def _record_solved_group(category_index: int, selected_words: Set[str]) -> None:
    """
    ...
    st.session_state.remaining = [
        word for word in st.session_state.remaining if word not in selected_words
    ]
...
def _handle_correct_guess(selected_words: Set[str]) -> None:
    """
    Handle a correct guess by recording the solved group and checking win condition.

    Args:
        selected_words: Set of correctly guessed words.
    """
    puzzle: Puzzle = st.session_state.puzzle
    category_index = next(iter({puzzle.word_to_category[w] for w in selected_words}))
    category = puzzle.categories[category_index]

    _record_solved_group(category_index, selected_words)
...
def _handle_incorrect_guess() -> None:
    """Handle an incorrect guess by incrementing mistakes and updating feedback."""
    ...
    # Clear selection after wrong guess to prompt rethinking
    st.session_state.selected = set()
...
def submit_guess() -> None:
    ...
    if _is_correct_guess(selected, puzzle):
        _handle_correct_guess(selected)
    else:
...
def render_status() -> None:
    """Display mistakes remaining, shuffle button, and feedback messages."""
...
def _render_word_button(word: str, is_selected: bool, is_locked: bool) -> None:
    """
    Render a single word button in the grid.
...
def render_grid() -> None:
    ...
    Dynamically adjusts grid rows based on remaining words and displays
    a submit button that is enabled only when exactly 4 words are selected.
    """
    st.subheader("Words")
    ...
    row_count = (word_count + WORDS_PER_ROW - 1) // WORDS_PER_ROW
    ...
    for _ in range(row_count):
        ...
        for column in columns:
            ...
            with column:
                _render_word_button(word, is_selected, is_locked)
...
def main() -> None:
    ...
    Initializes game state and renders all UI components.
    """
    init_state()
    render_header()
    render_solved()
