...
BALL_PADDLE_IMPACT_VERTICAL_FACTOR = 0.8
BALL_PADDLE_IMPACT_HORIZONTAL_MIN = 0.2
BALL_PADDLE_COLLISION_NUDGE = 0.01
...
class Paddle:
    ...
    def update(self, dt: float) -> None:
        ...
        """
        if self._direction == PADDLE_DIRECTION_IDLE:
            return
...
class Ball:
    ...
    def _handle_paddle_collision(self, paddle: Paddle, is_left: bool) -> None:
        ...
        paddle_half_height = paddle.rect.height / 2.0
        rel = (self.y - paddle.center_y) / paddle_half_height
        ...
        self.vy = rel * speed * BALL_PADDLE_IMPACT_VERTICAL_FACTOR

        # Horizontal component must point away from the paddle
        horiz_squared = max(0.0, speed * speed - self.vy * self.vy)
        horiz = max(BALL_PADDLE_IMPACT_HORIZONTAL_MIN, math.sqrt(horiz_squared))
        self.vx = horiz * (1 if is_left else -1)

        # Nudge the ball outside the paddle to avoid sticking
        if is_left:
            self.x = paddle.rect.right + self.radius + BALL_PADDLE_COLLISION_NUDGE
        else:
            self.x = paddle.rect.left - self.radius - BALL_PADDLE_COLLISION_NUDGE

    def _check_left_paddle_collision(
        self, ball_rect: pygame.Rect, left_paddle: Paddle
    ) -> None:
        """
        Check and handle collision with left paddle.

        Args:
            ball_rect: The ball's bounding rectangle.
            left_paddle: The left paddle.
        """
        if self.vx < 0 and ball_rect.colliderect(left_paddle.rect):
            self._handle_paddle_collision(left_paddle, is_left=True)

    def _check_right_paddle_collision(
        self, ball_rect: pygame.Rect, right_paddle: Paddle
    ) -> None:
        """
    ...
    def update(self, dt: float, left_paddle: Paddle, right_paddle: Paddle) -> None:
        ...
        self._check_left_paddle_collision(ball_rect, left_paddle)

        # Refresh rect after potential adjustment
        ball_rect = self.get_rect()
        self._check_right_paddle_collision(ball_rect, right_paddle)
