...
def _parse_coordinates(coords_raw: list) -> list[tuple[int, int]]:
    ...
    Args:
        coords_raw: Raw coordinate list from request payload.
...
def _get_remaining_words(game: GameState) -> list[str]:
    ...
    # Add spangram to front of list if not yet found
...
def hint():
    ...
    Returns:
        Response: JSON response with revealed hint details and updated game state.
    """
    session_id = _ensure_session()
    game = _get_game_state(session_id)
...
def reset():
    ...
    logger.info(f"Reset game session: {session_id}")
