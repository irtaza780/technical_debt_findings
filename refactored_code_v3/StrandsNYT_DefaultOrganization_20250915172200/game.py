...
class GameState:
    ...
    def _validate_selection_structure(self, coords: List[Coord]) -> Optional[Dict]:
        """
        Validate the structural integrity of a selection.
    ...
    def _check_spangram(self, word: str, coords: List[Coord]) -> Optional[Dict]:
        """
        ...
        Returns:
            Result dict if matches spangram, None otherwise.
        """
    ...
    def _check_theme_word(self, word: str, coords: List[Coord]) -> Optional[Dict]:
        """
        ...
        Returns:
            Result dict if matches theme word, None otherwise.
        """
    ...
    def _check_non_theme_word(self, word: str, coords: List[Coord]) -> Optional[Dict]:
        ...
        if len(word) >= MIN_DICTIONARY_WORD_LENGTH and word.lower() in NON_THEME_WORDS:
            if word not in self.non_theme_words:
                ...
                # Award hint for every N non-theme words found
    ...
    def try_commit_selection(self, coords: List[Coord]) -> Dict[str, Optional[str]]:
        ...
        Validates structure, checks against theme words and spangram using canonical paths,
        ...
        Returns:
            Dict with keys 'type', 'word', and 'coords' indicating result.
        """
        # Validate structural integrity
        structure_error = self._validate_selection_structure(coords)
        if structure_error:
    ...
    def _reveal_unfound_theme_word(self) -> Optional[Tuple]:
        """
        Reveal the next unfound theme word if hints available.

        Returns:
            Tuple of (word, coords, type) if revealed, None otherwise.
        """
        for theme_word in self.puzzle.theme_words:
            if theme_word not in self.found_words:
    ...
    def _reveal_spangram(self) -> Optional[Tuple]:
        """
        Reveal the spangram if hints available and not yet found.
    ...
    def reveal_hint(self) -> Optional[Tuple]:
        ...
        Prioritizes unfound theme words, then spangram if all themes are found.
        ...
        # Try to reveal an unfound theme word
        theme_hint = self._reveal_unfound_theme_word()
        if theme_hint:
            return theme_hint

        # If all themes found, try to reveal spangram
        return self._reveal_spangram()