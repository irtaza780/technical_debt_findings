...
# Shared puzzle instance (read-only)
...
def select():
    ...
    logger.info(f"Selection attempt in session {session_id}: {result}")
...
def hint():
    ...
    Returns:
        Response: JSON response with revealed hint and updated game state.
    """
    session_id = _ensure_session()
    game = _get_game_state(session_id)
    ...
    if revealed:
        ...
        logger.info(f"Hint revealed in session {session_id}: {hint_type}")
    else:
        result = {
            "type": "none",
            "word": None,
        ...
        logger.info(f"No hints available in session {session_id}")
...
def reset():
    ...
    Returns:
        Response: JSON response with fresh game state.
    """
    session_id = _ensure_session()
    ...
    return jsonify({
        "state": _serialize_state(_get_game_state(session_id))
    })
...
def remaining():
    ...
    Returns:
        Response: JSON response with list of remaining words and spangram.
    """
    session_id = _ensure_session()
    game = _get_game_state(session_id)
    
    # Find theme words not yet found
    remaining_words = [
        word for word in game.puzzle.theme_words
        if word not in game.found_words
    ]
    
    # Add spangram if not found
