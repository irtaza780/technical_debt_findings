...
class TicTacToeGUI:
    ...
    def _build_status_area(self) -> None:
        """Create and pack the status label area."""
    ...
    def _build_grid_area(self) -> None:
        """Create and pack the game grid with buttons."""
    ...
    def _create_grid_button(self, row: int, col: int) -> tk.Button:
        """
        Create a single grid button for a cell.
    ...
    def _build_control_area(self) -> None:
        """Create and pack the control buttons (New Game, Quit)."""
    ...
    def _is_game_over(self) -> bool:
        ...
        Returns:
            True if there is a winner or the board is full, False otherwise.
        """
        return self.game.winner is not None or self.game.is_draw()

    def _handle_game_state(self) -> None:
        """Handle the current game state after a move."""
        if self.game.winner is not None:
            ...
            messagebox.showinfo("Game Over", draw_message)
    ...
    def update_status(self) -> None:
        ...
        if self.game.winner is not None:
            ...
            # Color the status based on the winner
            winner_color = COLORS['x_fg'] if self.game.winner == SYMBOL_X else COLORS['o_fg']
            self.status_label.config(text=status_text, fg=winner_color)
        elif self.game.is_draw():
            self.status_label.config(
                text="It's a draw. Start a new game!",
                fg=COLORS['status_fg']
            )
        else:
            ...
            player_color = COLORS['x_fg'] if current_player == SYMBOL_X else COLORS['o_fg']
            self.status_label.config(text=status_text, fg=player_color)
    ...
    def highlight_winning_line(self) -> None:
        ...
        for row, col in self.game.winning_line:
            ...
            logger.debug(f"Highlighted winning cell at ({row}, {col})")
    ...
    def reset_game(self) -> None:
        ...
        # Clear all button displays and styles
        for row in self.buttons:
            for button in row:
                button.config(
                    text="",
