...
def get_font(font_type: str) -> Tuple[str, int, str]:
    ...
    if font_type not in font_specifications:
        logger.error(f"Font type '{font_type}' not found in font specifications")
        raise ValueError(f"Unknown font type: {font_type}")
    
    return font_specifications[font_type]


def get_player_color(player_symbol: str) -> str:
    """
    Get the display color for a given player symbol.
    
    Args:
        player_symbol: The player symbol ('X' or 'O').
    
    Returns:
        The hex color code for the player.
    
    Raises:
        ValueError: If the player symbol is not recognized.
    """
    player_colors = {
        SYMBOL_X: get_color('x_fg'),
        SYMBOL_O: get_color('o_fg'),
    }
    
    if player_symbol not in player_colors:
        logger.error(f"Unknown player symbol: {player_symbol}")
        raise ValueError(f"Unknown player symbol: {player_symbol}")
    
    return player_colors[player_symbol]