...
class TicTacToeGame:
    ...
    def _initialize_empty_board(self) -> None:
        """Create an empty board with all cells unoccupied."""
        self.board = [
    ...
    def _is_move_valid(self, row: int, col: int) -> bool:
        ...
        Returns:
            True if the move is within bounds, the cell is empty, and the game is not over.
        """
    ...
    def check_winner(self) -> Tuple[Optional[str], Optional[List[Tuple[int, int]]]]:
        ...
        Returns:
            A tuple of (winner_symbol, winning_line) where winner_symbol is 'X' or 'O' or None,
    ...
    def _check_anti_diagonal(self) -> Optional[List[Tuple[int, int]]]:
        ...
        Returns:
            A list of (row, col) tuples if the anti-diagonal is won, None otherwise.
        """
        if (
