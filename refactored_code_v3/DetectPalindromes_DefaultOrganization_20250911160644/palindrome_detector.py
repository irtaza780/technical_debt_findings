...
def _iter_sentences(line: str) -> Iterator[Tuple[int, int]]:
    ...
    while current_pos < line_length:
        if line[current_pos] in SENTENCE_TERMINATORS:
            ...
            # Include trailing quotes/brackets adjacent to sentence end
    ...
    # Yield remaining fragment if it exists
...
def _check_and_add_match(
    ...
    """
    Check if a segment is a palindrome and add it to results if it qualifies.
    
    Args:
        results: List to append the match to
        category: Type of palindrome ('word', 'sentence', or 'line')
        ...
        options: Detection options for normalization
    """
...
def detect_palindromes(text: str, options: DetectionOptions) -> List[PalindromeMatch]:
    ...
    Returns:
        List of PalindromeMatch objects representing detected palindromes
    """
    results: List[PalindromeMatch] = []
    lines = text.splitlines()

    for line_no, line in enumerate(lines, start=LINE_NUMBER_BASE):
        # Check entire line
