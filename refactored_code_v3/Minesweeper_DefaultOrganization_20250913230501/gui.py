...
class MinesweeperApp(tk.Frame):
    ...
    def _build_header(self):
        """Build the header frame with difficulty selector and status labels."""
        ...
        self.mines_label = tk.Label(
            header, text="Mines: 0", font=LABEL_FONT, bg=BG_COLOR_HEADER
        )
        ...
        self.timer_label = tk.Label(
            header, text="Time: 0", font=LABEL_FONT, bg=BG_COLOR_HEADER
        )
        self.timer_label.pack(side="left")

    def _build_board_frame(self):
        """Build the frame that will contain the game board."""
        self.board_frame = tk.Frame(
            self, bg=BG_COLOR_BOARD, padx=PADDING_STANDARD, pady=PADDING_STANDARD
        )
        self.board_frame.pack(side="top", expand=True)

    def _build_footer(self):
        """Build the footer frame with instructions."""
    ...
    def _create_cell_button(self, x, y):
        """
        Create a button for a single cell.
        ...
        # Bind click events with multiple aliases for cross-platform compatibility
    ...
    def _refresh_cells(self, coords, exploded_at):
        ...
        Args:
            ...
            exploded_at: Optional (x, y) tuple of the cell that triggered a mine explosion.
        """
        for x, y in coords:
    ...
    def _update_revealed_cell(self, btn, cell, x, y, exploded_at):
        ...
        if cell.has_mine:
            ...
            if adjacent_count > 0:
                color = NUMBER_COLORS.get(adjacent_count, FG_COLOR_MINE)
                btn.config(text=str(adjacent_count), disabledforeground=color)
            else:
                btn.config(text="")
    ...
    def _schedule_timer_tick(self):
        """Schedule the next timer update if the game is still playing."""
        if self.game.state != "playing":
            self._cancel_timer()
    ...
    def _update_timer_label(self, force_zero=False):
        ...
        Args:
            force_zero: If True, set timer to 0.
        """
        if force_zero:
