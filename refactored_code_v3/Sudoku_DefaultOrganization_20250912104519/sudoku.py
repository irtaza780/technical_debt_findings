...
class SudokuBoard:
    ...
    def to_list(self) -> Grid:
        ...
        Returns:
            A deep copy of the grid as a list of lists.
        """
    ...
    def find_empty(self, grid: Optional[Grid] = None) -> Optional[Tuple[int, int]]:
        ...
        Returns:
            Tuple of (row, col) for the first empty cell, or None if none exist.
        """
    ...
    def _is_valid_in_row(self, num: int, row: int, col: int, grid: Grid) -> bool:
        """
        Check if num is already present in the row (excluding current cell).
    ...
    def _is_valid_in_column(self, num: int, row: int, col: int, grid: Grid) -> bool:
        """
        Check if num is already present in the column (excluding current cell).
    ...
    def _is_valid_in_box(self, num: int, row: int, col: int, grid: Grid) -> bool:
        """
        Check if num is already present in the 3x3 box (excluding current cell).
    ...
    def generate_puzzle(
        ...
        Args:
            ...
            max_time: Maximum time in seconds to enforce uniqueness.
