...
class Game:
    ...
    def _update_paddle_directions(self) -> None:
        """Update movement directions based on current key states."""
        ...
        # Calculate directions: -1 (up), 0 (neutral), 1 (down)
    ...
    def reset_round(self, scored_by: str) -> None:
        ...
        serve_direction = -1 if scored_by == "right" else 1
        self.ball.reset(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, direction=serve_direction)
        self.ball_active = False
    ...
    def _render_scores(self) -> None:
        ...
        # Position scores at left and right quarters of screen
    ...
    def _render_cooldown_message(self) -> None:
        ...
        # Round up the display value
        remaining_seconds = int((remaining_ms / 1000.0) + COOLDOWN_ROUNDING_FACTOR)
        ready_text = f"Get ready... {remaining_seconds}"

        text_surface = self.info_font.render(ready_text, True, COLOR_DIM)
        center_x = SCREEN_WIDTH // 2
