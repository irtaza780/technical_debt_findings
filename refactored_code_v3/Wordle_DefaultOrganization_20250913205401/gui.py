...
class WordleGUI(tk.Tk):
    ...
    Provides an interactive interface with a 6x5 letter grid, input field,
    and on-screen keyboard with color-coded feedback.
    """
    ...
    def _build_board(self):
        """
        Build the 6x5 letter grid display for guesses.
        
        Creates labels for each cell and a status label for game messages.
        """
    ...
    @staticmethod
    def _create_board_cell() -> tk.Label:
        """
        Create a single board cell label with default styling.
        
        Returns:
            A configured tk.Label widget for displaying a letter.
        """
        return tk.Label(
            text=" ",
            width=CELL_WIDTH,
    ...
    def _build_input(self):
        """
        Build the input field and submit button.
        
        Creates an entry widget for user guesses and a submit button.
        """
    ...
    def _build_keyboard(self):
        ...
        Creates buttons for each letter arranged in standard QWERTY layout.
        """
    ...
    def _apply_colors_to_row(self, row_idx: int, guess: str, statuses: List[str]):
        """
        Update the colors of a row based on guess feedback.
        
        Args:
            row_idx: The row index to update.
        ...
        for col_idx in range(WORD_LENGTH):
            ...
            # Update label text and background color based on status
            label.config(text=letter)
            bg_color, fg_color = self._get_status_colors(status)
            label.config(bg=bg_color, fg=fg_color)

    @staticmethod
    def _get_status_colors(status: str) -> tuple:
        """
        Get the background and foreground colors for a given status.
        
        Args:
            status: The status string (CORRECT, PRESENT, or ABSENT).
            
        Returns:
            A tuple of (background_color, foreground_color).
        """
        if status == CORRECT:
    ...
    def _update_keyboard(self):
        ...
        Reflects which letters have been guessed and their status.
        """
    ...
    def _on_submit(self, event=None):
        ...
        Validates the guess, updates the board and keyboard, and checks for win/loss conditions.
        ...
        try:
            ...
            logger.error(f"Error submitting guess '{guess}': {error}")
            return
        
        # Update UI with guess results
    ...
    def _handle_game_end(self):
        """
        Handle game end conditions (win or loss).
        
        Updates status label and disables input if game is over.
        """
        if self.game.is_won:
            self._handle_win()
            ...
            # Game continues
            self.status_label.config(text=f"Attempts: {self.game.attempts_used}/{BOARD_ROWS}")

    def _handle_win(self):
        """
        Handle winning game state.
        
        Updates status label, shows congratulations message, and disables input.
        """
        message = f"Congratulations! You won in {self.game.attempts_used}/{BOARD_ROWS}."
    ...
    def _handle_loss(self):
        """
        Handle losing game state.
        
        Updates status label, shows game over message, and disables input.
        """
        secret_upper = self.game.secret.upper()
        message = f"Out of tries. The word was {secret_upper}."
        self.status_label.config(text=message)
        messagebox.showinfo("Game Over", f"The word was: {secret_upper}")
        self.entry.config(state="disabled")
        logger.info(f"Game lost. Secret word was: {secret_upper}")
