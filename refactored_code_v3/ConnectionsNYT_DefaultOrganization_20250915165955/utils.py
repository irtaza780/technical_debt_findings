...
def get_seeded_random(target_date: Optional[date] = None) -> random.Random:
    """
    Create a seeded Random instance for deterministic daily puzzle generation.
    
    Uses the provided date (or today's date if not specified) to seed the
    random number generator, ensuring consistent puzzle generation across
    multiple runs on the same day.
    
    Args:
        target_date: Date to seed from. Defaults to today's date if None.
    
    Returns:
        random.Random: A Random instance seeded by the date's epoch days.
    ...
    if not isinstance(target_date, date):
        ...
        raise TypeError(
            f"target_date must be a datetime.date instance, got {type(target_date).__name__}"
        )
