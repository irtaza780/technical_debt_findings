...
def _group_categories_by_difficulty(
    ...
    """
    Organize categories from the bank by difficulty level.
...
def _validate_word_uniqueness(
    ...
    Returns:
        Tuple of (is_valid, all_words). is_valid is True if all words are unique.
    """
    all_words: List[str] = []
    seen_words = set()
...
def _build_categories_from_definitions(
    ...
    Args:
        category_defs: List of category definitions.
        
    Returns:
        List of Category objects in the same order.
    """
    return [
        Category(
            name=cat_def.name,
...
def generate_daily_puzzle(date: Optional[dt_date] = None) -> Puzzle:
    ...
    for attempt in range(MAX_GENERATION_ATTEMPTS):
        ...
        is_valid, all_words = _validate_word_uniqueness(chosen_defs)
        if not is_valid:
            continue
