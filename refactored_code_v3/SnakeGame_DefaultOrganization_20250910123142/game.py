...
SHADOW_OFFSET = 2
PAUSE_OVERLAY_Y_OFFSET = 20
GAME_OVER_OVERLAY_Y_OFFSET = 40
...
class Game:
    ...
    def handle_input(self, event):
        ...
        # Handle movement direction (throttled)
        if self._is_direction_key(event.key):
    ...
    def _is_direction_key(self, key):
        """
        Check if a key is a direction input key.
        
        Args:
            key: pygame key constant
            
        Returns:
            True if key maps to a direction, False otherwise
        """
    ...
    def _handle_food_consumption(self):
        """Handle score increase and food respawning after consumption."""
        ...
        if not spawned:
            ...
            self.game_over = True
            self.victory = True
            logger.info("Victory: snake filled the board")
    ...
    def _draw_pause_overlay(self, surface):
        ...
        self._draw_centered_text(
            ...
            "Paused",
            self.font_big,
            y_offset=-PAUSE_OVERLAY_Y_OFFSET
        )
        
        hint_text = "Press P/Space to resume    R to restart    M for menu    ESC/Q to quit"
        hint_surf = self.font_small.render(hint_text, True, OVERLAY_TEXT_COLOR)
        hint_pos = hint_surf.get_rect(
            center=(surface.get_width() // 2, surface.get_height() // 2 + PAUSE_OVERLAY_Y_OFFSET)
        )
    ...
    def _draw_game_over_overlay(self, surface):
        ...
        self._draw_centered_text(
            ...
            title,
            self.font_big,
            y_offset=-GAME_OVER_OVERLAY_Y_OFFSET
        )
    ...
    def _draw_centered_text(self, surface, text, font, y_offset=0):
        """
        Draw text centered on the surface with a shadow effect.
        ...
        center_y = surface.get_height() // 2 + y_offset
        
        # Draw shadow
        shadow_surf = font.render(text, True, TEXT_SHADOW)
        shadow_pos = shadow_surf.get_rect(center=(center_x + SHADOW_OFFSET, center_y + SHADOW_OFFSET))
        surface.blit(shadow_surf, shadow_pos)
        
        # Draw text
        text_surf = font.render(text, True, TEXT_COLOR)
        text_pos = text_surf.get_rect(center=(center_x, center_y))
