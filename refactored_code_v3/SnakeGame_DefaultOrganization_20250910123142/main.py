...
def _handle_keyboard_input(event, difficulties):
    ...
    if KEYBOARD_SHORTCUT_START <= event.key <= KEYBOARD_SHORTCUT_END:
        shortcut_index = event.key - KEYBOARD_SHORTCUT_START
        if 0 <= shortcut_index < len(difficulties):
            return difficulties[shortcut_index]
...
def _handle_mouse_input(event, buttons, difficulties):
    ...
    Returns:
        str or None: Selected difficulty name or "continue" if no selection
    """
...
def _draw_difficulty_menu(screen, title_font, buttons, hint_font, start_y):
    """
...
def choose_difficulty(screen):
    ...
    while True:
        ...
        _draw_difficulty_menu(screen, title_font, buttons, hint_font, start_y)
...
def main():
    ...
    Initializes Pygame, displays the difficulty menu, and runs the game loop.
