import logging
from dataclasses import dataclass
from typing import Tuple

# Configure logging
logger = logging.getLogger(__name__)

# Movement constants
MIN_COORDINATE = 0
MAX_COORDINATE = 100
MIN_HEALTH = 0


@dataclass
class Entity:
    """Base class for all entities in the roguelike game.
    
    Attributes:
        x: Horizontal position on the game map.
        y: Vertical position on the game map.
        hp: Current health points of the entity.
    """
    x: int
    y: int
    hp: int

    def move(self, delta_x: int, delta_y: int) -> None:
        """Move the entity by the specified delta values.
        
        Args:
            delta_x: Change in horizontal position.
            delta_y: Change in vertical position.
        """
        new_x = self.x + delta_x
        new_y = self.y + delta_y
        
        # Validate coordinates are within bounds
        if self._is_valid_position(new_x, new_y):
            self.x = new_x
            self.y = new_y
            logger.debug(f"{self.__class__.__name__} moved to ({self.x}, {self.y})")
        else:
            logger.warning(
                f"Invalid move to ({new_x}, {new_y}). "
                f"Coordinates must be between {MIN_COORDINATE} and {MAX_COORDINATE}."
            )

    def take_damage(self, damage: int) -> None:
        """Reduce entity health by the specified damage amount.
        
        Args:
            damage: Amount of damage to apply.
        """
        if damage < 0:
            logger.warning("Damage cannot be negative. Ignoring.")
            return
        
        self.hp = max(MIN_HEALTH, self.hp - damage)
        logger.debug(f"{self.__class__.__name__} took {damage} damage. HP: {self.hp}")

    def heal(self, amount: int) -> None:
        """Restore entity health by the specified amount.
        
        Args:
            amount: Amount of health to restore.
        """
        if amount < 0:
            logger.warning("Healing amount cannot be negative. Ignoring.")
            return
        
        self.hp += amount
        logger.debug(f"{self.__class__.__name__} healed {amount} HP. Total HP: {self.hp}")

    def get_position(self) -> Tuple[int, int]:
        """Get the current position of the entity.
        
        Returns:
            Tuple of (x, y) coordinates.
        """
        return (self.x, self.y)

    def is_alive(self) -> bool:
        """Check if the entity is still alive.
        
        Returns:
            True if health is greater than zero, False otherwise.
        """
        return self.hp > MIN_HEALTH

    @staticmethod
    def _is_valid_position(x: int, y: int) -> bool:
        """Validate that coordinates are within acceptable bounds.
        
        Args:
            x: Horizontal coordinate to validate.
            y: Vertical coordinate to validate.
            
        Returns:
            True if both coordinates are within bounds, False otherwise.
        """
        return (MIN_COORDINATE <= x <= MAX_COORDINATE and 
                MIN_COORDINATE <= y <= MAX_COORDINATE)


@dataclass
class Player(Entity):
    """Player character entity in the roguelike game.
    
    Inherits movement and health management from Entity base class.
    """
    pass


@dataclass
class Monster(Entity):
    """Monster entity in the roguelike game.
    
    Inherits movement and health management from Entity base class.
    """
    pass