...
class UI:
    ...
    def _draw_separator_line(
        ...
        Args:
            surface: Pygame surface to draw on.
            ui_rect: Rectangle defining the UI area boundaries.
            y: Y-coordinate for the line.
        """
        start_x = ui_rect.x + UI_PADDING
        end_x = ui_rect.right - UI_PADDING
        pygame.draw.line(surface, C.COLOR_UI_HL, (start_x, y), (end_x, y))

    def _draw_player_stats(
        self, surface: pygame.Surface, x0: int, y: int, player: Player, level: int
    ) -> int:
        """
        Draw player statistics section.
    ...
    def _draw_last_encounter(
        ...
        """
        Draw last encountered monster information.
    ...
    def _wrap_and_draw_message(
        ...
        """
        Draw a message with word wrapping to fit UI width.
    ...
    def _draw_controls(
        self, surface: pygame.Surface, x0: int, y: int
    ) -> None:
        """
        Draw control instructions section.
    ...
    def draw(
        ...
        controls_y = C.WINDOW_HEIGHT - CONTROLS_OFFSET_FROM_BOTTOM
        self._draw_separator_line(screen, ui_rect, controls_y)
        controls_y += SEPARATOR_SPACING
        self._draw_controls(screen, x0, controls_y)