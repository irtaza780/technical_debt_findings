...
class CrosswordModel:
    ...
    def _read_across(self, row_idx: int, col_idx: int) -> Tuple[int, str]:
        """
        Read an across entry starting at the given position.
    ...
    def _read_down(self, row_idx: int, col_idx: int) -> Tuple[int, str]:
        """
        Read a down entry starting at the given position.
    ...
    def place_answer(self, number: int, direction: str, word: str) -> Tuple[bool, str]:
        ...
        if user_answer != entry.answer:
            mismatch_details = self._get_mismatch_details(entry, user_answer, direction)
            if mismatch_details:
                return False, f"Letters conflict with filled cells ({mismatch_details})."
            return False, "Incorrect answer."

        # Fill letters into grid
        self._fill_entry(entry, direction)
        entry.filled = True
    ...
    def _get_mismatch_details(
        ...
        """
        Generate details about letter conflicts.
        ...
        Returns:
            Formatted string of conflicts, or empty string if none.
        """
        mismatch_positions: List[Tuple[int, str, str]] = []

        for idx in range(entry.length):
            row_idx = entry.row + (idx if direction == DOWN_DIRECTION else 0)
            col_idx = entry.col + (idx if direction == ACROSS_DIRECTION else 0)
            existing = self.current_grid[row_idx][col_idx]

            if existing and existing != EMPTY_CELL and existing != user_answer[idx]:
                mismatch_positions.append((idx + 1, existing, user_answer[idx]))
    ...
    def _fill_entry(self, entry: CrosswordEntry, direction: str) -> None:
        """
        Fill an entry's answer into the current grid.
        ...
        for idx in range(entry.length):
            row_idx = entry.row + (idx if direction == DOWN_DIRECTION else 0)
            col_idx = entry.col + (idx if direction == ACROSS_DIRECTION else 0)
            self.current_grid[row_idx][col_idx] = entry.answer[idx]
