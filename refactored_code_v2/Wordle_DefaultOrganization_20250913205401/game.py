...
def _update_keyboard_status(
    ...
    """
    Update keyboard status for a letter with the best-known status.

    Uses STATUS_ORDER to determine which status is "better" (more informative).
    A CORRECT status always beats PRESENT, which beats ABSENT.

    Args:
        keyboard_status: Dictionary mapping letters to their best-known status.
        letter: The letter to update.
        new_status: The new status to consider.
    """
    current_status = keyboard_status.get(letter)

    # Only update if new status is better (higher order value)
...
class WordleGame:
    ...
    Attributes:
        ...
        keyboard_status: Mapping of each guessed letter to its best-known status.
    """
    ...
    def get_keyboard_status(self) -> Dict[str, str]:
        """
        Return the best-known status for each guessed letter.
