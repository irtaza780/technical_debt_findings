...
class GameUI:
    ...
    def _setup_top_section(self) -> None:
        ...
        self.status_label.pack(side=tk.LEFT, padx=FRAME_PADDING_X, pady=FRAME_PADDING_Y)
    ...
    def _setup_play_area(self) -> None:
        ...
        self.log_text.pack(
            fill=tk.BOTH,
            expand=True,
            padx=HAND_FRAME_PADDING,
            pady=HAND_FRAME_PADDING
        )
    ...
    def _setup_bottom_section(self) -> None:
        ...
        self.hand_frame.pack(side=tk.TOP, fill=tk.X, padx=HAND_FRAME_PADDING, pady=HAND_FRAME_PADDING)

        self.controls_frame = tk.Frame(self.bottom_frame)
        self.controls_frame.pack(side=tk.BOTTOM, fill=tk.X, padx=HAND_FRAME_PADDING, pady=HAND_FRAME_PADDING)
    ...
    def process_ai_bidding_loop(self) -> None:
        ...
        """
        if self.game.phase != BIDDING_PHASE:
            if self.game.phase == PLAY_PHASE:
    ...
    def on_bid(self, value: int) -> None:
        ...
        if self.game.phase == BIDDING_PHASE:
            self.enable_bidding_controls_if_needed()
            self.root.after(AI_BIDDING_DELAY_MS, self.process_ai_bidding_loop)
        elif self.game.phase == PLAY_PHASE:
            self.after_bidding_to_play()
    ...
    def after_bidding_to_play(self) -> None:
        ...
        """
        self._disable_all_bidding_buttons()
    ...
    def _disable_all_bidding_buttons(self) -> None:
        """Disable all bidding control buttons."""
    ...
    def ai_play_loop(self) -> None:
        ...
        """
        if self.game.phase != PLAY_PHASE:
            if self.game.phase == FINISHED_PHASE:
    ...
    def enable_play_controls_if_needed(self) -> None:
        """
        Enable or disable play controls based on game state.
        Pass is disabled at the start of a trick.
        """
        is_human_turn = (
            self.game.phase == PLAY_PHASE and
    ...
    def on_play(self) -> None:
        """Handle human play action with selected cards."""
        if self.game.phase != PLAY_PHASE:
            return
    ...
    def end_round_actions(self) -> None:
        """Disable controls and display round result."""
        self.play_button.config(state=tk.DISABLED)
        self.pass_button.config(state=tk.DISABLED)
        self._disable_all_bidding_buttons()
