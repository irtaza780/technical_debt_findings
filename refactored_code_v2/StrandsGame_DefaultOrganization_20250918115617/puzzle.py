...
class StrandPuzzle:
    ...
    def _create_segments_for_phrase(
        ...
        """
        Create segment objects for a phrase and add them to the all_segments list.
    ...
    def _split_into_segments(s: str, rng: random.Random) -> List[str]:
        ...
        if n <= 3:
            return [s]
        if n == MIN_PHRASE_LENGTH_FOR_SPLIT:
            return [s[:2], s[2:]]

        parts = StrandPuzzle._build_segments_iteratively(s, n, rng)
        
        # Ensure at least two parts if possible
        if len(parts) == 1 and n >= MIN_PHRASE_LENGTH_FOR_SPLIT:
            return [s[: n // 2], s[n // 2 :]]
        
        return parts

    @staticmethod
    def _build_segments_iteratively(s: str, n: int, rng: random.Random) -> List[str]:
        """
        Iteratively build segments while avoiding remainder of length 1.
        ...
        while i < n:
            remaining = n - i
            length = StrandPuzzle._calculate_segment_length(remaining, rng)
    ...
    def _calculate_segment_length(remaining: int, rng: random.Random) -> int:
        ...
        if remaining - length == REMAINDER_THRESHOLD:
...
def verify_merge(puzzle: StrandPuzzle, selected_ids: List[str]) -> Dict:
    ...
    segments = _resolve_segments(puzzle, selected_ids)
    if segments is None:
    ...
    if len(phrase_indices) != 1:
        return _handle_multi_phrase_error(puzzle, segments)
...
def _resolve_segments(
    ...
    Args:
        ...
        selected_ids: List of segment IDs to resolve.
        
    Returns:
        List of resolved segments, or None if any ID is invalid.
    """
    ...
    for seg_id in selected_ids:
        if seg_id in seg_by_id:
            segments.append(seg_by_id[seg_id])
        else:
            # Try to reconstruct from phrase map
            found = _segment_from_any(puzzle, seg_id)
            if not found:
                return None
...
def _handle_multi_phrase_error(puzzle: StrandPuzzle, segments: List[Segment]) -> Dict:
    """
    Handle error when segments come from different phrases.
    
    Args:
        puzzle: The puzzle being solved.
        segments: The selected segments from different phrases.
    ...
    if nearest and ratio >= SIMILARITY_THRESHOLD:
...
def _verify_phrase_segments(
    ...
    if is_increasing:
        return _handle_increasing_order(puzzle, segments, phrase_index, target_seg_ids)
    ...
    return _handle_general_invalid(puzzle, segments)


def _handle_increasing_order(
    puzzle: StrandPuzzle,
    segments: List[Segment],
    phrase_index: int,
    ...
    """
    Handle case where segments are in increasing order.
    
    Args:
        puzzle: The puzzle being solved.
        segments: Segments in increasing order.
        phrase_index: Index of the phrase.
...
def _handle_general_invalid(puzzle: StrandPuzzle, segments: List[Segment]) -> Dict:
    """
    Handle general invalid merge with helpful suggestion.
