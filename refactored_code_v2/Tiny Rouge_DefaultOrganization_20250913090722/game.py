...
class Game:
    ...
    def _get_movement_delta(key: int) -> Tuple[int, int]:
        """Convert a key code to movement deltas.
    ...
    def fight_monster_at(self, x: int, y: int) -> None:
        """Engage in combat with a monster at the given position.
        
        Applies damage to the player and removes the monster from the map.
    ...
    def open_chest_at(self, x: int, y: int) -> None:
        """Open a chest at the given position and heal the player.
    ...
    def _draw_entity_at(x: int, y: int, color: Tuple[int, int, int]) -> None:
        """Draw a single entity at the given position.
