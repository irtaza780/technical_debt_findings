...
class GameState:
    ...
    def modify_relationship(self, character: str, delta: int) -> None:
        """
        Modify a character relationship by a delta value.

        Args:
            character: The name of the character.
            delta: The amount to change the relationship by.
        """
        current = self.relationships.get(character, 0)
        self.relationships[character] = current + delta
        logger.info(f"Modified {character} relationship by {delta} (now {self.relationships[character]})")
    ...
    def get_relationship(self, character: str) -> int:
        ...
        Returns:
            The relationship value, or 0 if not set.
        """
    ...
    def meets_relationship_minimum(self, character: str, minimum: int) -> bool:
        ...
        Returns:
            True if relationship >= minimum, False otherwise.
        """
    ...
    def modify_variable(self, var_name: str, delta: int) -> None:
        """
        Modify a game variable by a delta value.

        Args:
            var_name: The name of the variable.
            delta: The amount to change the variable by.
        """
        current = self.variables.get(var_name, 0)
        self.variables[var_name] = current + delta
        logger.info(f"Modified variable {var_name} by {delta} (now {self.variables[var_name]})")
    ...
    def lacks_flags(self, flag_names: List[str]) -> bool:
        """
        Check if none of the flags in a list are set.
