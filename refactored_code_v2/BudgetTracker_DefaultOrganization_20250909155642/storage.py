...
class ExcelStorage:
    ...
    def _autosize_columns(ws: Worksheet) -> None:
        ...
        for row in ws.iter_rows(values_only=True):
            for column_index, cell_value in enumerate(row, start=1):
                ...
                current_max = column_widths.get(column_index, 0)
                column_widths[column_index] = max(current_max, len(cell_text))
    ...
    def load_model(self) -> BudgetModel:
        ...
        Returns:
            BudgetModel: The loaded budget model.
        """
        if not os.path.exists(self.filepath):
            self._initialize_empty_file()
    ...
    def _load_transactions(self, wb: Workbook) -> List[Transaction]:
        ...
        for row in trans_sheet.iter_rows(values_only=True):
            ...
            try:
                ...
                logger.error(f"Unexpected error loading transaction: {e}")
    ...
    def _build_workbook(self, model: BudgetModel) -> Workbook:
        ...
        This method is used by save_model and export_copy to avoid duplicating
        workbook construction logic.
        
        Args:
            model: The BudgetModel to convert to a workbook.
            
        Returns:
            Workbook: A new workbook with all model data.
        """
        wb = Workbook()
    ...
    def save_model(self, model: BudgetModel) -> None:
        ...
        Creates a fresh workbook from the model and writes it to the
    ...
    def export_copy(self, model: BudgetModel, dest_path: str) -> None:
        """
        Export a copy of the current model to a destination path.
        
        Creates a new workbook from the model and saves it to the specified
        destination without modifying the active storage filepath.
        
        Args:
            model: The BudgetModel to export.
            dest_path: The destination file path for the export.
        """
        wb = self._build_workbook(model)
        wb.save(dest_path)
