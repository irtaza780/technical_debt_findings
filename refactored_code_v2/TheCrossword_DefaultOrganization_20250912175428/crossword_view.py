...
CLUES_SECTION_PADY = (0, 8)
STATUS_COLOR_SUCCESS = "#0a0"
STATUS_COLOR_ERROR = "#a00"
STATUS_COLOR_DEFAULT = "#333"
GRID_CELL_BG_EMPTY = "white"
GRID_CELL_BG_BLOCKED = "black"
CLUES_BG_COLOR = "#fafafa"
DIRECTION_ACROSS = "A"
DIRECTION_DOWN = "D"
FILLED_MARK = "✓ "
EMPTY_MARK = "  "
...
class CrosswordView:
    ...
    def _build_main_layout(self) -> None:
        """Build the main layout with left and right frames."""
    ...
    def _build_right_panel(self) -> None:
        ...
        self._build_clues_section()
        self._build_status_section()

    def _build_controls_section(self) -> None:
        """Build the answer input controls section."""
    ...
    def _build_action_buttons(self) -> None:
        """Build the submit and reset action buttons."""
    ...
    def _build_clues_section(self) -> None:
        """Build the clues display section."""
        self.clues_frame = ttk.LabelFrame(self.right_frame, text="Clues", padding=CONTROLS_PADDING)
        ...
        self.across_text.pack(fill=tk.BOTH, expand=True, pady=CLUES_SECTION_PADY)
        self._configure_clues_text(self.across_text)
        ...
        self._configure_clues_text(self.down_text)

    def _configure_clues_text(self, text_widget: tk.Text) -> None:
        """
        Configure a text widget for read-only clues display.
        
        Args:
            text_widget: The text widget to configure.
        """
        text_widget.configure(state=tk.DISABLED)
        text_widget.configure(background=CLUES_BG_COLOR)

    def _build_status_section(self) -> None:
        """Build the status message display."""
        ...
        for r in range(rows):
            ...
            for c in range(cols):
                lbl = tk.Label(
                    ...
                    relief="solid",
                    font=CELL_FONT,
                    bg=GRID_CELL_BG_EMPTY,
                )
                lbl.grid(row=r, column=c, sticky="nsew", padx=CELL_PADDING, pady=CELL_PADDING)
    ...
    def update_grid(self, cell_value_at: Callable[[int, int], str]) -> None:
        ...
        for r in range(self.rows):
            for c in range(self.cols):
                ...
                # Blocked cells are black; others show the letter or remain empty
                if val == "#":
                    lbl.configure(text="", bg=GRID_CELL_BG_BLOCKED)
                else:
                    lbl.configure(text=val if val else "", bg=GRID_CELL_BG_EMPTY)
    ...
    def _format_clue_list(self, entries: List[CrosswordEntry], direction: str) -> str:
        ...
        Args:
            ...
            direction: Direction indicator (for logging purposes).
            
        Returns:
            Formatted clue string.
        """
        if not entries:
        ...
        for entry in entries:
            ...
            lines.append(f"{mark}{entry.number}{direction} ({entry.length}): {entry.clue}")
    ...
    def set_status(self, message: str, ok: bool = True) -> None:
        ...
        color = STATUS_COLOR_SUCCESS if ok else STATUS_COLOR_ERROR
        self.status_label.configure(foreground=color)

    def bind_submit(self, callback: Callable[[], None]) -> None:
        """
        Bind a callback to the submit button.
