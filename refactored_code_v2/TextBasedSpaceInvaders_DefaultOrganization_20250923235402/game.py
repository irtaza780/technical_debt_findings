...
class AlienFleet:
    ...
    def reached_bottom(self, y_limit: int) -> bool:
        ...
        Returns:
            True if any alien reached the limit, False otherwise.
        """
        if len(self.aliens) == 0:
            return False
...
class Game:
    ...
    def __init__(self):
        """Initialize pygame, game state, sprite groups, and pre-rendered assets."""
        self._init_pygame()
    ...
    def _init_background(self):
        """Create pre-rendered starfield background with deterministic random seed."""
    ...
    def _init_fonts(self):
        """Initialize font objects for different UI elements."""
    ...
    def _spawn_fleet(self):
        """Create and add alien fleet to the game."""
    ...
    def _handle_quit_events(self, event: pygame.event.EventType):
        """
    ...
    def _handle_state_transition_events(self, event: pygame.event.EventType):
        """
        Handle events that transition game state (start/restart).
    ...
    def handle_events(self):
        """Process all input events and update game state accordingly."""
        for event in pygame.event.get():
            self._handle_quit_events(event)
            if not self.running:
                return
            self._handle_state_transition_events(event)

        # Handle continuous shooting input during gameplay
        if self.state == GAME_STATE_PLAYING:
            keys = pygame.key.get_pressed()
            if keys[pygame.K_SPACE]:
                self._attempt_player_shoot()

    def _attempt_player_shoot(self):
        """Attempt to fire a bullet from the player, respecting fire rate limits."""
    ...
    def update(self):
        """Update game logic: entities, collisions, and win/lose conditions."""
        if self.state == GAME_STATE_PLAYING:
            keys = pygame.key.get_pressed()
    ...
    def resolve_collisions(self):
        """Detect and handle all collision events: bullets hitting aliens and aliens hitting player."""
        self._resolve_bullet_alien_collisions()
        self._resolve_player_alien_collisions()

    def _resolve_bullet_alien_collisions(self):
        ...
        if hits:
            ...
            logger.debug(f"Destroyed {destroyed_aliens} aliens. Score: {self.score}")

    def _resolve_player_alien_collisions(self):
        """Handle collisions between player and aliens with invulnerability check."""
        now = pygame.time.get_ticks()
        if now >= self.player_safe_until_ms:
            # Check collision without destroying aliens to preserve game challenge
            collided = pygame.sprite.spritecollide(self.player, self.aliens, dokill=False)
            if collided:
                self.lose_life()

    def lose_life(self):
        """Deduct one life, reset player position, and grant temporary invulnerability."""
        self.lives -= 1
    ...
    def draw(self):
        """Render all game elements: background, sprites, HUD, and state overlays."""
    ...
    def _draw_lives(self):
        """Draw remaining lives as ship icons in top-right corner."""
        ...
        for i in range(self.lives):
            x_offset = i * LIFE_ICON_SPACING
            self._draw_life_icon(HUD_LIVES_ICONS_X + x_offset, HUD_LIVES_ICONS_Y)

    def _draw_life_icon(self, x: int, y: int):
        """
        Draw a single life icon (small ship silhouette).

        Args:
            x: X-coordinate of icon center.
            y: Y-coordinate of icon top.
        """
        points = [
    ...
    def _draw_state_overlay(self):
        """Draw state-specific overlay messages (start, game over, victory)."""
