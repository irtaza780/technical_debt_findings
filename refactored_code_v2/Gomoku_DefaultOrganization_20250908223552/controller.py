...
class GameController:
    ...
    def undo_move(self) -> None:
        ...
        If the undo stack is empty, this method returns early.
        """
        ...
        logger.info(f"Undone move by player {PLAYER_NAMES[player]}")
        self._refresh_view()

    def redo_move(self) -> None:
        """
        Redo the most recently undone move and recompute game state.
        
        If the game is already over or the redo stack is empty, this method returns early.
        """
        if self.game_over:
        ...
        logger.info(f"Redone move by player {PLAYER_NAMES[player]} at ({row}, {col})")
    ...
    def _check_and_handle_win(self, row: int, col: int) -> bool:
        """
        Check if the last move resulted in a win and handle the end state.
        ...
        if winner != NO_WINNER:
            ...
            logger.info(f"Player {PLAYER_NAMES[winner]} wins!")
            self._refresh_view()
            if self._view:
    ...
    def _check_and_handle_draw(self) -> bool:
        """
        Check if the board is full (draw condition) and handle the end state.
