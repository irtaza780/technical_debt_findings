...
class GameState:
    ...
    def _capture_sequences_from(self, pos: Coord, piece: Piece) -> List[List[Coord]]:
        ...
        A man becomes a king upon reaching the king row, ending the capture sequence.
        ...
        def recurse(
            ...
            for dr, dc in directions:
                ...
                # Check bounds for both middle and landing squares
                ...
                # Valid jump: opponent piece in middle, empty landing square
                if mid_piece and mid_piece.color == self._opponent(color) and landing_empty:
                    ...
                    if becomes_king:
                        # Kinging ends the capture sequence
    ...
    def _check_kinging(self, piece: Piece, row: int, color: str) -> bool:
        """
        Check if a piece should be kinged and apply the kinging.

        Args:
            piece: The piece to potentially king.
            row: The current row of the piece.
            color: The color of the piece.

        Returns:
            True if the piece was kinged, False otherwise.
        """
        if piece.king:
            return False
        ...
        if color == RED_PLAYER and row == RED_KING_ROW:
            piece.king = True
            return True
        elif color == BLACK_PLAYER and row == BLACK_KING_ROW:
            piece.king = True
            return True
    ...
    @staticmethod
    def _is_valid_move_sequence(seq: List[Coord], legal_moves: List[List[Coord]]) -> bool:
        """
        Check if a move sequence matches one of the legal moves.
        ...
        Returns:
            True if the sequence matches a legal move, False otherwise.
        """
        return any(GameState._same_path(seq, legal) for legal in legal_moves)
    ...
    def _apply_capture_sequence(self, seq: List[Coord]) -> None:
        ...
        for i in range(len(seq) - 1):
            ...
            # Remove the captured piece (midpoint between from and to)
            r1, c1 = from_pos
            r2, c2 = to_pos
            mid_r = (r1 + r2) // 2
            mid_c = (c1 + c2) // 2
    ...
    def _post_move_kinging(self, dest: Coord) -> None:
        ...
        Args:
            dest: The destination coordinate of the move.
        """
        r, c = dest
        piece = self.board.get(r, c)
        if piece:
