...
def _serialize_claimed_cells(game: "GameState") -> list[dict]:
    """
    Serialize the claimed cells from a game state.
    
    Args:
        game (GameState): The game state to serialize.
...
def _serialize_state(game: "GameState") -> dict:
    ...
    Returns:
        dict: A dictionary containing all game state information.
    """
...
def _parse_coordinates(coords_data: list) -> list[tuple[int, int]]:
    """
    Parse and validate coordinate pairs from request data.
    
    Args:
        coords_data (list): List of coordinate pairs to parse.
...
def _get_remaining_words(game: "GameState") -> list[str]:
    """
    Get the list of words not yet found in the game.
    
    Args:
        game (GameState): The game state to check.
...
def state():
    ...
    Returns:
        Response: JSON response containing the serialized game state.
    """
...
def select():
    ...
    Returns:
        Response: JSON response with result and updated game state.
    """
    session_id = _ensure_session()
    game = _get_game_state(session_id)
...
def hint():
    ...
    Returns:
        Response: JSON response with revealed hint information and updated state.
    """
    session_id = _ensure_session()
    game = _get_game_state(session_id)
...
def reset():
    ...
    Returns:
        Response: JSON response with the fresh game state.
    """
    session_id = _ensure_session()
    _SESSIONS[session_id][GAME_KEY] = GameState(PUZZLE)
...
def remaining():
    ...
    Returns:
        Response: JSON response with list of remaining words.
    """
    session_id = _ensure_session()
    game = _get_game_state(session_id)
