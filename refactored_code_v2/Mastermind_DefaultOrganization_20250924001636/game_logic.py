...
class MastermindGame:
    ...
    def new_game(self) -> None:
        """
        Start a new game by generating a new secret code and resetting game state.
    ...
    def get_history(self) -> List[Dict[str, object]]:
        """
        Retrieve the complete history of guesses and feedback.
    ...
    def evaluate_guess(self, guess: List[str]) -> Tuple[int, int]:
        ...
        Returns:
            A tuple (exact_matches, partial_matches).
    ...
    def _calculate_matches(self, guess: List[str]) -> Tuple[int, int]:
        ...
        2. For remaining unmatched positions, count partial matches by comparing
           symbol frequencies in unmatched guess and secret.
    ...
    @staticmethod
    def _count_partial_matches(unmatched_secret: List[str], unmatched_guess: List[str]) -> int:
        """
        Count partial matches from unmatched symbols.

        For each symbol in the guess, count how many times it appears in the secret,
        limited by its frequency in the guess (to avoid double-counting).

        Args:
            unmatched_secret: Symbols from secret that weren't exact matches.
        ...
        # Sum the minimum count for each symbol present in both
        return sum(min(secret_counts[symbol], guess_counts[symbol]) for symbol in guess_counts)
