...
def start_event_loop(root: tk.Tk) -> None:
    """
    Start the Tkinter event loop for the application.
    ...
    root.mainloop()
    logger.info("Application event loop terminated")
...
def main() -> None:
    ...
    Creates the root window, initializes the application, and starts
    the event loop.
    """
    try:
        root = create_root_window()
        initialize_application(root)
        start_event_loop(root)
    except Exception as e:
